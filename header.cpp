...
u64 prim_throw(u64 v)
{
    ASSERT_TAG(v, STR_TAG, "(fail s); s is not a string")
    fatal_err(DECODE_STR(v));
    return V_NULL;
}
...
u64 applyprim_vector(u64 lst)
{
    ...
    u64* buffer = alloc(512*sizeof(u64));
    mem_count += 512*sizeof(u64);
...
u64 prim_system(u64 v)
{
    ASSERT_TAG(v, STR_TAG, "(system s); s is not a string")
    system(DECODE_STR(v));
...
u64 prim_string_45length(u64 str)
{
    ASSERT_TAG(str, STR_TAG, "(string-length s); s is not a string")

    int n = strlen(DECODE_STR(str));
    return ENCODE_INT(n);
}

u64 prim_string_45append(u64 a, u64 b)
{
    ASSERT_TAG(a, STR_TAG, "(string-append a b); a is not a string")
    ASSERT_TAG(b, STR_TAG, "(string-append a b); b is not a string")

    int n1 = strlen(DECODE_STR(a));
    int n2 = strlen(DECODE_STR(b));

    char *new_s = (char *) malloc(n1+n2+1);
    mem_count += n1+n2+1;
    if (mem_count > MAX_MEM)
        fatal_err("EXCEEDED MAXIMUM AMOUNT OF MEMORY");
    strcpy(new_s, DECODE_STR(a));
    strcat(new_s, DECODE_STR(b));
    return ENCODE_STR(new_s);
}

u64 prim_string_45ref(u64 s, u64 i)
{
    ASSERT_TAG(s, STR_TAG, "(string-ref s i); s is not a string")
    ASSERT_TAG(i, INT_TAG, "(string-ref s i); i is not an integer")

    int len = strlen(DECODE_STR(s));
    if (DECODE_INT(i) >= len) {
        printf("string-ref: out of bounds\nindex given: %d\nvalid range: [0,%d]\n", DECODE_INT(i), len-1);
        exit(1);
    }
    ...
    if (mem_count > MAX_MEM)
        fatal_err("EXCEEDED MAXIMUM AMOUNT OF MEMORY");
    strcpy(new_s, DECODE_STR(s));
    char c = new_s[DECODE_INT(i)];
    return ENCODE_CHAR(c);
}

u64 prim_substring(u64 s, u64 a, u64 b)
{
    ASSERT_TAG(s, STR_TAG, "(substring str a b); str is not a string")
    ASSERT_TAG(a, INT_TAG, "(substring str a b); a is not an integer")
    ASSERT_TAG(a, INT_TAG, "(substring str a b); b is not an integer")

    if (DECODE_INT(b) < DECODE_INT(a))
        fatal_err("substring ending index is less than starting index");

    if (DECODE_INT(a) < 0)
        fatal_err("substring begin index is less than 0");

    int len = strlen(DECODE_STR(s));
    if (DECODE_INT(b) > len)
    {
        fatal_err("substring ending index is out of bounds");
    }

    char *ret_val = (char *)malloc(DECODE_INT(b)-DECODE_INT(a));
    mem_count += DECODE_INT(b)-DECODE_INT(a);
    if (mem_count > MAX_MEM)
        fatal_err("EXCEEDED MAXIMUM AMOUNT OF MEMORY");
    strcpy(ret_val, &DECODE_STR(s)[DECODE_INT(a)]);
    ret_val[DECODE_INT(b)-1] = '\0';
    return ENCODE_STR(ret_val);
}
...
u64 prim_string_45set(u64 s, u64 k, u64 c)
{
    ...
    if (i > len)
    {
        fatal_err("string-set! index out of bounds");
    }
...
{
    ASSERT_TAG(c, CHAR_TAG, "(ascii c); c is not a char")
