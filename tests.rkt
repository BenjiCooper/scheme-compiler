...
;; Testing apparatus for the final project
...
(define (test-top-level top-level-e)  
  (define val1 (eval-top-level top-level-e))   
  (define scm-e (proc->llvm (closure-convert (cps-convert (anf-convert (alphatize (assignment-convert (simplify-ir (desugar (top-level top-level-e))))))))))       
  (define val2 (eval-llvm scm-e))
  (if (equal? val1 val2)
      #t
      (begin
...
(define tests (map (path->test 'public) (tests-list "final_starter_tests")))
...
(define (get-code file) (with-input-from-file (string-append "tests/final_starter_tests/" file ".scm") read-begin #:mode 'text))
